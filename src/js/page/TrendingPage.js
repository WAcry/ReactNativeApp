...
class TrendingPage extends Component<Props> {
            ...
            if (item.checked) {
                tabs[`tab${index}`] = {
                    screen: props => <TrendingTabPage {...props} timeSpan={this.state.timeSpan} tabLabel={item.name}
                                                      theme={theme}/>,//   Component        @https://github.com/react-navigation/react-navigation/issues/2392
                    navigationOptions: {
                        title: item.name
                    }
                }
    ...
    _tabNav() {
        ...
        //  ：            top tab
        if (theme !== this.theme || !this.tabNav || !ArrayUtil.isEqual(this.preKeys, this.props.keys)) {//    ：             TabNavigator，  tab        
            this.theme = theme;
